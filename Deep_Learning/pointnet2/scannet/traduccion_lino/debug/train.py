...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 27 10:08:17 2022
...
'''
# BASE_DIR = os.path.dirname(os.path.abspath(__file__))
BASE_DIR = '/home/lino/Documentos/programas_pruebas_varias/PointNet/pointnet2'
...
if not LINO:
    ...
    #DATASET_PATH = '/mnt/lustre/scratch/nlsas/home/uvi/ir/lcc/nubes_de_puntos/nubes_artificiales/DATASETS_POINTNET2/autopista_seccion_transversal/DATASET__autopista_con_seccion_transversal'
    DATASET_PATH = '/home/lino/Documentos/programas_pruebas_varias/segmentacion_python/segmentacion_bosques/aumentacion_de_datos/Nubes_artificiales_generadas/nubes_buenas/autopista_bermas_senhales'
    
    TRAIN_DATA_PATH = DATASET_PATH +'/TRAIN'
    TEST_DATA_PATH = DATASET_PATH +'/TEST'    
    PREDICT_DATA_PATH = '/home/lino/Documentos/NDP_carreteras_Santarem_Dani'
    ...
    archivo_informacion_dataset = 'informacion_dataset.txt'
    with open(archivo_informacion_dataset) as f:
        lineas = f.readlines()
        tipo_de_nubes = lineas[1][15:-1]
        NUM_CLASSES = int(lineas[2][18:-1])
    f.close()
...
def train_one_epoch(sess, ops, train_writer,RUTA_LOG_ABSOLUTA,epoca):
    ...
    for ll in range(NUMERO_NUBES_ENTRENAMIENTO):
        ...
        TRAIN_DATASET = scannet_dataset.ScannetDataset(root=TRAIN_DATA_PATH, npoints=NUM_POINT, split='train',LINO=LINO,NUM_CLASSES=NUM_CLASSES,downsampleamos=False,contador_dataset=contador_dataset_entrenamiento,tamanho_batch = BATCH_SIZE,tipo_de_nubes=tipo_de_nubes)
...
def eval_one_epoch(sess, ops, test_writer,epoch,RUTA_LOG_ABSOLUTA):
    ...
    for ll in range(NUMERO_NUBES_TESTEO):
        ...
        TEST_DATASET = scannet_dataset.ScannetDataset(root=TEST_DATA_PATH, npoints=NUM_POINT, split='test',LINO=LINO,NUM_CLASSES=NUM_CLASSES,downsampleamos=False,contador_dataset=contador_dataset_testeo,tamanho_batch = BATCH_SIZE, tipo_de_nubes=tipo_de_nubes)
...
def eval_one_epoch_PREDICCION_NUBE_REAL(sess, ops, test_writer,epoch,RUTA_LOG_ABSOLUTA):
    ...
    for ll in range(NUMERO_NUBES_PREDICCION):
        ...
        PREDICT_DATASET = scannet_dataset.ScannetDataset(root=PREDICT_DATA_PATH, npoints=NUM_POINT, split='prediccion',LINO=LINO,NUM_CLASSES=NUM_CLASSES,downsampleamos=False,contador_dataset=0,tamanho_batch = 1,tipo_de_nubes=tipo_de_nubes,archivo='L055-1-M01-S2-C1_s_RGB_REDNAP_AJ_RECORTADA.npy')
        # contador_dataset_testeo += BATCH_SIZE

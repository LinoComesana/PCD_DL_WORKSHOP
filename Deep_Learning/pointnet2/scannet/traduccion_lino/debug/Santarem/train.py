...
# sesion:
nombre_sesion = 'PAPER_autopista_Santarem__def_4_layers_PN++_default'
...
def eval_one_epoch_PREDICCION_NUBE_REAL(sess, ops, test_writer,epoch,RUTA_LOG_ABSOLUTA):
    ...
    for ll in range(NUMERO_NUBES_PREDICCION):
        ...
        PREDICT_DATASET = scannet_dataset.ScannetDataset(root=PREDICT_DATA_PATH, npoints=NUM_POINT, split='prediccion',LINO=LINO,NUM_CLASSES=NUM_CLASSES,downsampleamos=False,contador_dataset=0,tamanho_batch = 1,tipo_de_nubes=tipo_de_nubes,archivo='Nube_artificial_%i.npy'%ll,algunas_clases_fusionadas=False)
        # contador_dataset_testeo += BATCH_SIZE
